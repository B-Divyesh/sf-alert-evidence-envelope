...
The demo creates a random server workspace that expires after 24 hours. Its identifier and last rendered envelope use the browser keys `demo:alert-evidence-envelope:session` and `demo:alert-evidence-envelope:preview`. The workspace exists only in server memory. It cannot read or write the SQLite route configuration or delivery history.
